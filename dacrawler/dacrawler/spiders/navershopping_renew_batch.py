...
class NavershoppingListSpider(scrapy.Spider):
    name = 'navershopping_product_batch_kjy'
    channel = CHANNEL
...
class NavershoppingReviewSpider(scrapy.Spider):
    ...
    name = 'navershopping_review_batch'
    custom_settings = {
    ...
    def start_requests(self):
        ids = self.db.select('product','id,url,site_productID,grouped_by_shop', f"channel = 'navershopping' and category= '{self.category}' and crawled='F'")
        for id in ids:
