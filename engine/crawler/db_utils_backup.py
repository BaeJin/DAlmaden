...
class RequestTask:
    ...
    def make_task_table(self,channel,search_keyword,until_date,n_periods,n_crawl):
        ...
        if channel in self.period_differ_channel:
            ...
            df_request_crawl.at[0,'n_crawl'] = n_crawl
    ...
    def request_to_task(self):
        ...
        ##request 를 선택한다.
        request_rows = pd.read_sql("select * from crawl_request where task_ids is null",con=sql_alchemy_engine.connect())
        for idx in request_rows.index:
            ...
            for idx in tasks_df.index:
                ...
                ##중복된 task_id 가 존재하지 않는 경우에는 직전에 삽입하고 돌려받은 task_id 를 가져와서 request 와 mapping 한다.
                if task_id is not None:
                    db.insert_withoutDuplication('crawl_request_task',['request_id','task_id'],request_id=request_id,task_id=task_id)
                    self.crawl_total_contents_num(task_id,keyword,channel,from_date,to_date)
    ...
    def crawl_total_contents_num(self,task_id,keyword,channel,from_date,to_date):
        if channel=='naverblog':
            obj = CrawlLibnaverBlog(task_id,keyword,from_date,to_date)
            obj.crawl_total()
            del obj
        if channel=='navernews':
            obj = CrawlLibnaverNews(task_id,keyword,from_date,to_date)
            obj.crawl_total()
            del obj
        if channel=='instagram':
            obj = CrawlLibInstagram(task_id,keyword)
            obj.crawl_total()
            del obj
        if channel=='navershopping':
            obj = CrawlLibNavershopping(task_id,keyword)
            obj.crawl_total()
            del obj


        if channel=='amazon':
            obj = CrawlLibAmazon(task_id,keyword)
            obj.crawl_total()
            del obj
            pass
