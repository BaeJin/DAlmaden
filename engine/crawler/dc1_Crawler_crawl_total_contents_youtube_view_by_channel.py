...
db=Sql("datacast2")
status = "GR"
contents_row = db.select('crawling_status_youtube_view','*','contents_status="GR" and is_channel=1')
...
for idx,row in enumerate(contents_row):
    ...
    if real_contents_status=='GR' and (n_reply is not None and n_reply>0):
    ...
    elif real_contents_status=='GR' and (n_reply is None or n_reply<=0):
