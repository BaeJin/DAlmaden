...
sql = "select * from htdocs where keyword=\'%s\' and channel=\'%s\' and publishtime>=\'%s\' and publishtime<=\'%s\' limit %d" % \
      (keyword, channel, startDate, endDate, quantity)
curs.execute(sql)
rows = curs.fetchall()
text = ''
...
print("Subject %s count = %d, score = %d, average = %5.2f" % (subj, subjCount, subjScore, (subjScore / subjCount)))
...
pd_phrase.to_excel("pol/phrase/pol_phrase_"+keyword+"_"+startDate+"_"+endDate+".xlsx", encoding="utf-8", index=True)
...
sql = "insert into pol_rate(keyword, function, channel, fun_count , pos_num, neg_num, pos_rate, neg_rate, stdate, endate, nurl, crawltime)"\
+ "values (\'%s\', \'%s\', \'%s\', \'%d\',\'%d\', \'%d\', \'%s\', \'%s\', \'%s\', \'%s\', \'%d\', \'%s\')" % \
(keyword, subj, channel, subjCount, subjPosCount, subjNegCount, pos_rate, neg_rate, startDate, endDate, quantity, fftime)
curs.execute(sql)
conn.commit()
conn.close()